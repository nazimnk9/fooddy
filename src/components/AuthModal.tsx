...
import { Lock, User, X, Eye, EyeOff, AlertCircle } from "lucide-react";
...
export function AuthModal({ children, open, onOpenChange }: AuthModalProps) {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMsg = error.data?.detail || "No active account found with the given credentials";
            setAlertConfig({
                open: true,
                title: "Login Failed",
                message: errorMsg
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error.data) {
                const messages = [];
                if (error.data.email) messages.push(...error.data.email);
                if (error.data.phone) messages.push(...error.data.phone);
                if (error.data.detail) messages.push(error.data.detail);
